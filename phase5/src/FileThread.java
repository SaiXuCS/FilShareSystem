		...
		try
		{
			...
			do
			{
				...
				SecretKeySpec secretKeySpec = new SecretKeySpec(symmetricClientFile, "AES");
					...
					if(!tokenVerified.getServerID().equals(my_fs.serverID))	socket.close();
				...
				if(e.getMessage().equals("LFILES"))
				{
					...
					if(!compareToken(tokenVerified,(Token)usertoken))	socket.close();
				...
				if(e.getMessage().equals("UPLOADF"))
				{
					...
					else
					{
						...
						if(encrypted.getObjContents().get(2) == null) {
							...
							if(!compareToken(tokenVerified,(Token)yourToken))	socket.close();
					...
					if(!compareToken(tokenVerified,(Token)t))	socket.close();
