		...
		try
		{
			...
			do
			{
				...
				else if(message.getMessage().equals("DISCONNECT")) //Client wants to disconnect
				{
					...
					byte[] clientKey = (byte[])message.getObjContents().get(0); //Get the username
					...
					if(clientKey == null)
					{
						...
						try {
							if (my_gs.secrets.containsKey(username)) {
								byte[] w = my_gs.secrets.get(username);
								Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding", "BC");
								SecretKeySpec secretKeySpec = new SecretKeySpec(w, "AES");
								cipher.init(Cipher.DECRYPT_MODE, secretKeySpec, new IvParameterSpec(iv));
								byte[] decryptedKey = cipher.doFinal(clientKey);
								//System.out.println("\nDecrypted Key:");
								//for(byte b: decryptedKey) System.out.print(b);
						/* retrieve public key */
								KeyFactory keyFactory = KeyFactory.getInstance("DH", "BC");
								EncodedKeySpec publicKeySpec = new X509EncodedKeySpec(decryptedKey);
								PublicKey publicKey = keyFactory.generatePublic(publicKeySpec);

								DHParameterSpec dhParams = new DHParameterSpec(g, p);
								KeyPairGenerator dhKeyGen = KeyPairGenerator.getInstance("DH", "BC");
								dhKeyGen.initialize(dhParams, new SecureRandom());
								KeyAgreement dhKeyAgree = KeyAgreement.getInstance("DH", "BC");
								KeyPair dhPair = dhKeyGen.generateKeyPair();
								dhKeyAgree.init(dhPair.getPrivate());
								dhKeyAgree.doPhase(publicKey, true);
								key = dhKeyAgree.generateSecret();
								for (int i = 0; i < 16; i++) {
									symmetricKey[i] = key[i];
								}
								//System.out.println("\nThe DH key: ");
								//for(byte b:key) System.out.print(b);
								cipher.init(Cipher.ENCRYPT_MODE, secretKeySpec);
								//System.out.println("\nOriignal Key:");
								//for(byte b: dhPair.getPublic().getEncoded()) System.out.print(b);
								byte[] encryptedKey = cipher.doFinal(dhPair.getPublic().getEncoded());
								iv = cipher.getIV();
								response = new Envelope("OK");
								response.addObject(encryptedKey.clone());
								response.addObject(iv.clone());
								output.writeObject(response);
							} else {
		...
		if(my_gs.userList.checkUser(requester))
		{
			...
			if(temp.contains("ADMIN"))
			{
				...
				else
				{
					...
					byte[] w = digest.digest(password.getBytes(StandardCharsets.UTF_8));		//generate shared secret w
					my_gs.secrets.put(username,w);
					return true;
				}
			}
			else
